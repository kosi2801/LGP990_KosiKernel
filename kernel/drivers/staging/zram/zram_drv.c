...
#if defined(CONFIG_ZSWAP_OLD_COMPRESSION)
#include <linux/lzo.h>
#else
#include "csnappy.h"
#endif 
			...
			enum zram_stats_index idx, s64 val)
{
	...
	preempt_disable();
	stats = SHIFT_PERCPU_PTR(zram->stats, __my_cpu_offset);
...
#if defined(CONFIG_ZRAM_OLD_COMPRESSION)
		ret = lzo1x_decompress_safe(zmem, xv_get_object_size(zmem),
					bio_mem, &zlen);
#else
		ret = csnappy_decompress_noheader(zmem, xv_get_object_size(zmem),
		        bio_mem, &zlen);
#endif
...
static int zram_write(struct zram *zram, struct bio *bio)
{
	...
	bio_for_each_segment(bvec, bio, i) {
...
#ifndef CONFIG_ZRAM_OLD_COMPRESSION
		unsigned char *end;
#endif
		...
		ret = lzo1x_1_compress(bio_mem, PAGE_SIZE, src, &zlen,
					zworkmem);
#else
	        end = csnappy_compress_fragment(bio_mem, PAGE_SIZE, src, 
                                zworkmem, CSNAPPY_WORKMEM_BYTES_POWER_OF_TWO);
	        zlen = end - src;
#endif
...
#else
		per_cpu(compress_workmem, cpu) = kzalloc(
					CSNAPPY_WORKMEM_BYTES, GFP_KERNEL);
#endif
